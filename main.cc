...
void LoadParams(const string &inputsFile)
{
    ...
    if(read_paramfiles){
        //cout<<"read"<<endl;
        instring =stringstream(Params::getInstance().getWalkParams(inputsFile));
        input = &(instring);
    }
    else{
        infile = ifstream(inputsFile,ios::in);
        input = &(infile);
    }
    ...
    //ofstream debug_out("./debug_para");
    //for(auto i:namedParams)
    //   debug_out<<i.first<<"\t"<<i.second<<"\n";
    //debug_out.close();
...
void Run()
{
    ...
    if (agentType == "naoagent") {
        ...
        behavior = new Test(teamName, uNum, namedParams, rsg);
    }
    else {
    ...
    //ofstream out = ofstream("./debug_msg",ios::out)   ; 
    string msg;
    while (gLoop)
    {
        GetMessage(msg);

...
protected:
    ...
    int woplay();//得到我方最近球员号
    int whogo();//得到谁补上来
    double woplayjuli(VecPosition target);//得到我方离球最近的球员距离
    double opplayjuli(VecPosition target);//得到对方球员离球最近的球员的距离
    bool  Comparison();//得到是我方球员带球还是对方球员带球
    void pass();
    SkillType closeOtherGoal();
    SkillType closeOtherCorner();
    SkillType kickik(vector<pair<VecPosition,float>>* scoreSheet);
    vector<pair<VecPosition,float>>*  scorePointList();
    bool judgePlayerCollision(VecPosition target);
    double GetRand();
    double GetFand();    
    virtual SkillType Anticollision1(double x,double y);
    virtual SkillType Anticollision2(double x,double y,double z,double w);
    virtual SkillType Anticollision3(double y,double z);
    virtual SkillType Anticollision4(double x,double y);
    virtual SkillType Anticollision5(double x,double y,double z);
    virtual SkillType Anticollision6(double x,double y);
    virtual SkillType Anticollision7(double x);//边线球员
    virtual SkillType Anticollision8(double x);
...
//***************************************************************************************************
    ...
    SkillType attack();
    SkillType stop();
    SkillType angle_bisecton();
    SkillType cylinder();
    SkillType mark(int m);
    SkillType def();
    SkillType attackposit();
    SkillType defendposit();
    SkillType attackposit1(int number);
    SkillType peopletopeople();
    SkillType goalkeeper();
    SkillType save();
    SkillType avoid();
    SkillType sideplayer();
    int belong();
    int kic();
    int kicc();
    bool avoidball();
